...
| Rust | `ckgame-rust/` | Rust 1.70+，Cargo | `cd ckgame-rust && cargo build --release && cargo run --release` |

六个版本共享同一套机制设计：`GameSimulation` 主循环（按日推进、月度结算）+ `GameAPI` 统一接口（snapshot/action）+ TUI 命令行文字界面，存档均为 JSON（`saves/` 目录）。
...
- [docs/rust/](docs/rust/README.md) — Rust 版
...
注：各版本功能完成度略有差异。Go/Rust/C++ 版当前仅完成 `core/` 基础模块与演示入口，核心引擎与 TUI 待实现。详见对应版本文档。
