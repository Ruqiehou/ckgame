...
export class CouncilMonthlyResult {
  ...
  public piety: number = 0;
  public controlGain: number = 0;
  public developmentChance: number = 0;
  public claimProgress: number = 0;
  public logs: string[] = [];
}
...
export class Council {
  public readonly ruler: number;
  public chancellor: number = NONE_ID;
  public marshal: number = NONE_ID;
  public steward: number = NONE_ID;
  public spymaster: number = NONE_ID;
  public chaplain: number = NONE_ID;
  public tasks: Map<CouncilPosition, CouncilTask> = new Map();
  ...
  set(pos: CouncilPosition, who: number): void {
    switch (pos) {
      case CouncilPosition.CHANCELLOR: this.chancellor = who; break;
      case CouncilPosition.MARSHAL: this.marshal = who; break;
      case CouncilPosition.STEWARD: this.steward = who; break;
      case CouncilPosition.SPYMASTER: this.spymaster = who; break;
      case CouncilPosition.COURT_CHAPLAIN: this.chaplain = who; break;
    }
  }

  /** Return all appointed positions with their holder ids. */
  members(): [CouncilPosition, number][] {
    const out: [CouncilPosition, number][] = [];
  ...
  autoAppoint(candidates: number[][]): void {
    ...
    if (this.ruler !== NONE_ID) used.add(this.ruler);

    const pick = (scoreIdx: number): number => {
      let best = NONE_ID;
      let bestS = -999;
    ...
    for (const [pos, who] of this.members()) {
      ...
      switch (task) {
        ...
        default: {
          ...
          switch (pos) {
            case CouncilPosition.CHANCELLOR: r.prestige += skills.diplomacy * 0.15; break;
            case CouncilPosition.MARSHAL: r.controlGain += skills.martial * 0.1; break;
            case CouncilPosition.STEWARD: r.gold += skills.stewardship * 0.2; break;
            case CouncilPosition.SPYMASTER: r.prestige += skills.intrigue * 0.1; break;
            case CouncilPosition.COURT_CHAPLAIN: r.piety += skills.learning * 0.2; break;
          }
          break;
        }
      }
