...
export class EventEngine {
  public catalog: EventDef[];
  public pending: EventInstance[] = [];
  ...
  private cooldownKey(eventId: number, character: number): string {
  ...
  tickCooldowns(): void {
    ...
    for (const [key, v] of this.cooldowns) {
      if (v > 1) next.set(key, v - 1);
    }
    this.cooldowns = next;
  }

  private eligible(world: World, who: number, ev: EventDef): boolean {
    const c = world.characters.get(who) as Character | undefined;
    if (!c) return false;
    if (c.life !== 'ALIVE') return false;
    if (ev.requiresRuler && !c.isRuler) return false;
    if (ev.requiresAdult) {
      // Simple adult check: age >= 16
      const age = world.date.year() - c.birth.year();
      if (age < 16) return false;
    }
    if (c.gold < ev.minGold) return false;
    if (ev.requiresMarried && c.spouses.length === 0) return false;
    return true;
  }
  ...
  dailyCheck(world: World, characters: number[]): void {
    for (const who of characters) {
      for (const ev of this.catalog) {
        ...
        if (!this.eligible(world, who, ev)) continue;
        const p = Math.max(0.005, Math.min(0.35, ev.weight * 0.02));
        if (this.rng() < p) {
          this.pending.push(new EventInstance(
            ev.id,
            who,
            ev.title,
            ev.description,
          ...
          if (ev.major) break; // Only one major event per day per character
        }
      }
    }
  }
  ...
  resolveChoice(world: World, instance: EventInstance, choiceId: number): void {
    ...
    const c = world.characters.get(instance.character) as Character | undefined;
    const name = c?.name ?? '?';
  ...
  autoResolveAll(world: World): void {
    ...
    for (const inst of toResolve) {
      ...
      for (const c of inst.choices) {
        if (c.aiWeight > (best?.aiWeight ?? -Infinity)) best = c;
      }
      if (best) this.resolveChoice(world, inst, best.id);
    }
  }
  ...
  static builtinEvents(): EventDef[] {
    ...
    events.push(new EventDef(1, '丰收之年', '领地迎来大丰收，农民献上额外税赋。',
      3.0, 365, false, [
    ...
    events.push(new EventDef(2, '宫廷丑闻', '朝中流传关于你的闲言碎语。',
      2.0, 180, false, [
    ...
    events.push(new EventDef(3, '狩猎意外', '狩猎时坐骑受惊，你险些摔伤。',
      1.5, 400, true, [
    ...
    events.push(new EventDef(4, '虔诚的捐赠', '主教请求你资助修建教堂。',
      2.0, 300, false, [
    ...
    events.push(new EventDef(5, '封臣的抱怨', '一位封臣公开抱怨赋税过重。',
      2.5, 200, false, [
    ...
    events.push(new EventDef(8, '瘟疫阴影', '附近村落出现疫病。',
      1.2, 500, true, [
    ...
    events.push(new EventDef(9, '边境劫掠', '强盗侵扰边境村庄。',
      2.3, 180, false, [
    ...
    events.push(new EventDef(10, '学者来访', '一位学者带来稀有抄本。',
      1.8, 320, false, [
    ...
    events.push(new EventDef(11, '节日庆典', '臣民请求举办丰年节。',
      2.0, 200, false, [
    ...
    events.push(new EventDef(12, '决斗挑战', '一位傲慢贵族要求决斗。',
      1.3, 450, true, [
    ...
    events.push(new EventDef(13, '商路开通', '商会提议开辟新商路。',
      1.7, 360, false, [
    ...
    events.push(new EventDef(14, '神秘访客', '戴兜帽的陌生人带来可疑情报。',
      1.6, 280, false, [
    ...
    events.push(new EventDef(17, '丰收祭典', '秋季丰收，农民请求举办祭典。',
      2.0, 250, false, [
    ...
    events.push(new EventDef(18, '密谋败露', '你发现了一起针对你的暗杀阴谋。',
      1.2, 500, true, [
    ...
    events.push(new EventDef(19, '商队遇袭', '你的商队在途中遭到劫掠。',
      1.8, 220, false, [
    ...
    events.push(new EventDef(20, '贵族求婚', '一位贵族向你或你的子女求婚。',
      1.6, 350, false, [
    ...
    events.push(new EventDef(21, '建筑火灾', '你领内的一座建筑突发火灾。',
      1.4, 260, false, [
    ...
    events.push(new EventDef(22, '外敌入侵', '边境传来警报，敌军正在劫掠村庄。',
      1.8, 180, true, [
    ...
    events.push(new EventDef(23, '丰收祭典', '秋季丰收，农民请求举办祭典。',
      2.0, 250, false, [
        new EventChoice(0, '举办盛大祭典', [new Effect('gold', -25), new Effect('prestige', 12), new Effect('stress', -5)], 8),
        new EventChoice(1, '简单庆祝', [new Effect('gold', -8), new Effect('prestige', 5)], 6),
        new EventChoice(2, '取消祭典', [new Effect('prestige', -8), new Effect('stress', 5)], 3),
    ...
    events.push(new EventDef(24, '密谋败露', '你发现了一起针对你的暗杀阴谋。',
      1.2, 500, true, [
        new EventChoice(0, '公开审判', [new Effect('prestige', 15), new Effect('stress', 5)], 7),
        new EventChoice(1, '秘密处决', [new Effect('stress', 10), new Effect('gold', -10)], 6),
        new EventChoice(2, '宽恕并监视', [new Effect('stress', 5)], 5),
    ...
    events.push(new EventDef(25, '商队遇袭', '你的商队在途中遭到劫掠。',
      1.8, 220, false, [
        new EventChoice(0, '派兵护卫', [new Effect('gold', -20), new Effect('prestige', 8)], 8),
        new EventChoice(1, '赔偿损失', [new Effect('gold', -15)], 5),
        new EventChoice(2, '追捕劫匪', [new Effect('gold', -10), new Effect('prestige', 12)], 6),
    ...
    events.push(new EventDef(26, '贵族求婚', '一位贵族向你或你的子女求婚。',
      1.6, 350, false, [
    ...
    events.push(new EventDef(27, '瘟疫蔓延', '领地内爆发瘟疫，百姓流离失所。',
      1.3, 600, true, [
    ...
    events.push(new EventDef(28, '比武大会', '领地举办比武大会，各地骑士云集。',
      1.9, 300, false, [
    ...
    events.push(new EventDef(29, '密信截获', '你截获了一封密信，揭露了邻国的军事计划。',
      1.5, 400, false, [
    ...
    events.push(new EventDef(30, '饥荒预警', '连续干旱可能导致饥荒。',
      1.7, 320, false, [
