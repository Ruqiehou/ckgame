...
import { crownAuthorityTaxBonus } from './Laws.js';
import { MapGraph } from './MapGraph.js';
import { Title } from './Title.js';
import { terrainDevelopmentCap } from './Terrain.js';
import { tradeRouteKey } from './TradeRoute.js';
...
export class World {
    ...
    createCharacter(name, dynasty, gender, birth) {
        const cid = this.nextChar++;
        const c = new Character(cid, name, dynasty, gender, birth);
        c.baseAttrs = new AttributeSet(this.nextInt(4, 14), this.nextInt(4, 14), this.nextInt(4, 14), this.nextInt(4, 14), this.nextInt(4, 14), this.nextInt(4, 14));
    ...
    // ---------- 关系与封建 ----------
    setParents(child, father, mother) {
        ...
        if (father !== NONE_ID) {
            ...
            if (p !== undefined && !p.children.includes(child))
    ...
    marry(a, b) {
        ...
        if (ca === undefined || cb === undefined)
            return false;
        if (!ca.isAlive() || !cb.isAlive())
            return false;
        if (ca.gender === cb.gender)
            return false;
        if (!ca.spouses.includes(b))
            ca.spouses.push(b);
        if (!cb.spouses.includes(a))
            cb.spouses.push(a);
        this.pushLog(`${ca.name} 与 ${cb.name} 成婚`);
        return true;
    }
    grantTitle(titleId, holder) {
        ...
        t.setHolder(holder);
        if (!h.heldTitles.includes(titleId))
    ...
    setVassal(vassalTitle, liegeTitle) {
        ...
        if (v.deFactoLiege !== NONE_ID && v.deFactoLiege !== liegeTitle) {
            ...
            if (oldLiege !== undefined) {
                ...
                if (idx >= 0)
                    oldLiege.deFactoVassals.splice(idx, 1);
            }
        }
        v.deFactoLiege = liegeTitle;
        if (!l.deFactoVassals.includes(vassalTitle))
            l.deFactoVassals.push(vassalTitle);
        return true;
    }
    clearVassalLink(titleId) {
        const t = this.title(titleId);
        if (t === undefined)
            return;
    ...
    attachCountyToTitle(countyId, titleId) {
        const t = this.title(titleId);
        const c = this.map.get(countyId);
        if (t === undefined || c === undefined)
            return false;
        if (!t.counties.includes(countyId))
            t.counties.push(countyId);
    ...
    opinion(from, to) {
        ...
        if (a === undefined || b === undefined)
            return 0;
        let op = 0;
        if (a.dynasty !== NONE_ID && a.dynasty === b.dynasty)
            op += 15;
        op += (a.culture === b.culture) ? 10 : -5;
        op += (a.faith === b.faith) ? 10 : -20;
        if (a.father === to || a.mother === to || b.father === from || b.mother === from)
            op += 30;
        if (a.spouses.includes(to))
            op += 25;
        if (a.children.includes(to) || b.children.includes(from))
            op += 20;
        ...
        for (const tid of b.traits) {
            const t = byId.get(tid);
            if (t !== undefined)
                op += Math.floor(t.opinionOthers / 2);
        }
    ...
    modifyOpinion(from, to, delta) {
        ...
        if (cur === 0)
            c.opinionCache.delete(to);
        else
            c.opinionCache.set(to, cur);
        ...
        if (heir !== null) {
            ...
            if (law !== null && law.partitionEnabled) {
                ...
                for (const cid of c.children) {
                    ...
                    if (k !== undefined && k.isAlive())
                        livingKids.push(cid);
                }
                const heirs = [heir];
                for (const k of livingKids) {
                    if (k !== heir && heirs.length < 4)
                        heirs.push(k);
                }
    ...
    occupyCounty(countyId, newHolder) {
        ...
        if (tid === NONE_ID)
            return true;
        const t = this.title(tid);
        if (t === undefined || t.tier !== TitleTier.COUNTY)
            return true;
            ...
            for (const cid of t.counties) {
                ...
                for (const route of this.tradeRoutes) {
                    if (route.from === county.key || route.to === county.key) {
                        ...
                        let portBonus = 1.0;
                        if (county.hasPort)
                            portBonus *= (1.0 + county.portLevel * 0.2);
                        income += route.tradeVolume * exchangeRate * 0.1 * portBonus;
                        if (county.hasPort)
                            income += county.portIncome * 0.5;
                    }
                }
            }
        }
    ...
    triggerTradeEvent(route) {
        ...
        if (fromCounty !== undefined && fromCounty.tradeRouteProtected)
            protectionBonus *= 1.3;
        if (toCounty !== undefined && toCounty.tradeRouteProtected)
            protectionBonus *= 1.3;
    ...
    adjustExchangeRate(key, multiplier) {
    ...
    updateExchangeRates() {
        ...
        for (const route of routesSnapshot) {
            ...
            if (this.exchangeRates.has(key)) {
                ...
                if (fromCounty !== undefined)
                    maintenanceBonus *= fromCounty.getTradeRouteMaintenanceBonus();
                if (toCounty !== undefined)
                    maintenanceBonus *= toCounty.getTradeRouteMaintenanceBonus();
    ...
    processFertility() {
        ...
        for (const [fatherId, motherId] of couples) {
            ...
            if (!m.isAlive() || m.gender !== Gender.FEMALE)
                continue;
            if (!m.isAdult(this.date) || m.ageAt(this.date) > 45)
                continue;
            ...
            if (fa !== null && ma !== null && ch !== undefined) {
                ch.baseAttrs = new AttributeSet(Math.max(1, Math.floor((fa.diplomacy + ma.diplomacy) / 2) + this.nextInt(-2, 3)), Math.max(1, Math.floor((fa.martial + ma.martial) / 2) + this.nextInt(-2, 3)), Math.max(1, Math.floor((fa.stewardship + ma.stewardship) / 2) + this.nextInt(-2, 3)), Math.max(1, Math.floor((fa.intrigue + ma.intrigue) / 2) + this.nextInt(-2, 3)), Math.max(1, Math.floor((fa.learning + ma.learning) / 2) + this.nextInt(-2, 3)), Math.max(1, Math.floor((fa.prowess + ma.prowess) / 2) + this.nextInt(-2, 3)));
